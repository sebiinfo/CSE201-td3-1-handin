...
void merge_telemetry(double **telemetries,
   ...
   for (int i=0; i<tot_telemetries; i++){
      ...
      //Now we do a bubble sort based on time
    bool ok=true;
    while (ok==true)
    {
        ok=false;
        for (int i=0; i<global_telemetry_current_size;i+=3)
        {
            ...
            {
                if (global_telemetry[i]>global_telemetry[j]) //time
                {
                    swap(global_telemetry[i], global_telemetry[j]);
                    swap(global_telemetry[i+1], global_telemetry[j+1]);
                    swap(global_telemetry[i+2], global_telemetry[j+2]);
                    ok=true;
